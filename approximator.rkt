...
(command-line
 #:program "approximate"
 #:once-each [("-l" "--linear") "Use linear approximation" (linear-enabled #t)]
 [("-q" "--quadratic") "Use quadratic approximation" (quad-enabled #t)]
 [("-e" "--exponent") "Use exponential approximation" (exp-enabled #t)]
 [("-g" "--logarithm") "Use logarithmic approximation" (log-enabled #t)]
 [("-p" "--power") "Use power approximation" (pow-enabled #t)]
 [("-s" "--segment") "Use segment approximation" (seg-enabled #t)]
 [("--step") raw-step "Step between yielded X, default 1" (step (string->number raw-step))]
 [("--start") raw-start "Starting X, default 0" (start (string->number raw-start))]
 [("-c" "--count")
  raw-count
...
(module+ main
  ...
  (let*-values ([(more? get) (sequence-generate table)])
    ...
    (for/fold ([N 0]
               ...
               [n 0]
               #:result (void))
              ([x0 (in-generator (let loop ([x (- (start) (* 2 (step)))])
                                   (begin
                                     (yield x)
                                     (loop (+ x (step))))))]
               #:break (>= n (count)))
      ...
      (when (> N 1)
        ...
        (pretty-print x0)
        (when (linear-enabled)
          (pretty-print #:sep ";" ((linear N SX SXX SY SXY) x0)))
        (when (quad-enabled)
          (pretty-print #:sep ";" ((quadratic N SX SXX SY SXY SXXX SXXXX SXXY) x0)))
        (when (exp-enabled)
          (pretty-print #:sep ";" ((exponential N SX SXX SLnY SXLnY) x0)))
        (when (log-enabled)
          (pretty-print #:sep ";" ((logarithmic N SLnX SLnX2 SY SLnXY) x0)))
        (when (pow-enabled)
          (pretty-print #:sep ";" ((power N SLnX SLnX2 SLnY SLnXLnY) x0)))
        (when (seg-enabled)
          (pretty-print #:sep ";" ((segment Xs Ys) x0)))
        (newline))
      (if (more?)
